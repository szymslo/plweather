...
const mapNZero = 54.9;
const mapEFull = 24.27;
const mapNFull = 49.0;

const degN = 950 / Math.abs(mapNZero - mapNFull);
const degE = 1000 / Math.abs(mapEZero - mapEFull);

const rescaleLat = (lat) => {
  const rescaled = Math.round(Math.abs(lat - mapEZero) * degE * 100) / 100;
  return rescaled;
};

const rescaleLong = (long) => {
  const rescaled = Math.round(Math.abs(long - mapNZero) * degN * 100) / 100;
  return rescaled;
};

const Polmap = ({ data }) => {
  //'stations' is array which contains cities (names) with IMGW weather station
  const stations = [];
  data.forEach((el) => {
    stations.push(el.stacja);
  });

  //'allcities' is array of all cities in Poland with their coordinates
  // we need to add coordinates to stations
  const markers = allcities.filter((city) => {
    return stations.includes(city.name);
  });
